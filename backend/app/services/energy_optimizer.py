...
class EnergyOptimizer:
    ...
    def __init__(self, weather_api: WeatherAPI = None, carbon_tracker: CarbonTracker = None):
    ...
    def get_weather_forecast(self, hours: int = 48) -> Dict:
        """Get weather forecast directly from WeatherAPI"""
        try:
            forecast_days = max(1, hours // 24 + 1)
            weather_data = self.weather_api.fetch_weather_forecast(forecast_days)
    ...
    def get_carbon_impact(self, energy_saved_kwh: float, timestamp: str = None) -> Dict:
        """Calculate carbon impact directly from CarbonTracker"""
        try:
            ts_obj = None
            if timestamp:
                try:
            ...
            return self.carbon_tracker.calculate_carbon_impact(energy_saved_kwh, ts_obj)
        except Exception as e:
            self.logger.error(f"Failed to calculate carbon impact: {e}")
            return {}
    
    def get_weather_alerts(self) -> List[Dict]:
        """Get weather alerts directly from WeatherAPI"""
        try:
            weather_data = self.weather_api.fetch_weather_forecast(3)
            return self.weather_api.get_weather_alerts(weather_data)
        except Exception as e:
