...
class COIL2000DataPipeline:
    ...
    def __init__(self, data_path: str = None):
        ...
        self.feature_vocab = {}
        self.feature_info = {}
        self.categorical_lookup = {} # New for 0.0.8
        
        self.logger = logging.getLogger(__name__)
    
    def download_coil2000_dataset(self) -> pd.DataFrame:
        """Download COIL 2000 dataset or load local real version"""
        real_local_path = os.path.join(os.path.dirname(__file__), "coil2000_real.arff")
        
        if os.path.exists(real_local_path):
            self.logger.info(f"Loading real COIL 2000 dataset from local file: {real_local_path}")
            try:
                data, meta = arff.loadarff(real_local_path)
                df = pd.DataFrame(data)
                self.logger.info(f"Real dataset loaded successfully: {df.shape}")
                return df
            except Exception as e:
                self.logger.error(f"Failed to load local real dataset: {e}")

        # Fallback to download or mock
        self.logger.info("Local real dataset not found or failed, attempting download from OpenML...")
        try:
            # OpenML API URL for COIL 2000 dataset (ID: 298)
            url = "https://www.openml.org/data/v1/download/298/coil2000.arff"
    ...
    def build_vocabulary(self, df: pd.DataFrame):
        """Build vocabulary for categorical features and initialize StringLookup layers"""
        for feature in self.categorical_features:
            ...
            # Create and adapt StringLookup layer for 0.0.8
            lookup_layer = StringLookup(vocabulary=unique_values, mask_token=None)
            self.categorical_lookup[feature] = lookup_layer
            
        self.logger.info(f"Built vocabulary and StringLookup for {len(self.categorical_features)} categorical features")
